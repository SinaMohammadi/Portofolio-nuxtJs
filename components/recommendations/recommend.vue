...
<template>
  <div
    class="flex text-start p-8 flex-col gap-3 border border-solid border-purple-900 hover:bg-purple-900 hover hover:bg-opacity-20 rounded-2xl"
  >
    <svg
    ...
    <h6 class="text-sm sm:text-base text-white opacity-60">
  {{ recommend.commend }}
    </h6>
    <div class="flex gap-2">
      <img class="w-10 h-10 sm:w-14 sm:h-14" :src="recommend.image" alt="" />
      <div>
        <h4 class="text-xs sm:text-xl text-white">{{ recommend.person }}</h4>
        <h5 class="text-xs sm:text-base text-purple-600">
          {{ recommend.deg }}
        </h5>
      </div>
    </div>
